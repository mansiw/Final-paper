...
polintrplot <- ggplot(data = comparepolintr, aes(x=TIME, y=avgpolintr)) + geom_line(aes(colour=cntry)) + scale_x_continuous(breaks = c(2002, 2004, 2006, 2008, 2010, 2012, 2014), labels=c("2002","2004","2006","2008","2010","2012", "2014"), limits = c(2002,2014)) + labs(x="Years", y="Average Political Interest", color = "Countries") + ggtitle("Average Political Interest for Youth (Select Countries)") + guides(fill = guide_legend(title = "Countries"))
...
FittedPolIntrPlot <- ggplot(data=fittedpolintr2, aes(x=Education, y=Fitted, fill=Unemployed)) + geom_bar(stat="identity", position=position_dodge()) + labs(x="Level of Education", y="Predicted Level of Political Interest") + ggtitle("Predicted Level of Political Interest") + theme(plot.margin = unit(c(1,4,1,1), "cm"), legend.title = element_text(size = 10)) + scale_fill_discrete(name="Employment <br> Status", breaks=c(0, 1), labels=c("Employed", "Unemployed")) 
...
Looking at a different measure of political engagement, we also looked at whether being unemployed makes you less likely to vote. As the below graph shows, the likelihood that a young person in Europe voted in the last election decreases if that person is also unemployed (with a value of 0 on the x-axis meaning an individual is employed, and a value of 1 on the x-axis meaning an individual is unemployed). 
...
votingplot <- ggplot(ESSData, aes(x=unempdummy, y=votedummy)) + geom_point(shape=1, position=position_jitter(width=.0001,height=.0001)) + scale_x_continuous(breaks = c(0,1)) + stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE) + labs(x="Employment Status (Employed or Unemployed)", y="Likelihood of voting") + ggtitle("Effect of Unemployment on Voting Behaviour")
...
The effects we found in our regressions (as well as in the further regressions completed in our longer paper) are statistically significant, meaning we can say there is a real effect on both a young person's political interest and their tendency to vote that is generated by being unemployed, and that this effect can't be written away as random variation. Still, as seen in the above tables, the actual amount of change as a result of being unemployed is not huge. For that reason, we can't say that that unemployment significantly impacts a young person's level of political interest, but we can say that, based our data and analysis, it definitely has an effect.
