...
votingplot <- ggplot(ESSData, aes(x=unempdummy, y=votedummy)) + geom_point(shape=1, position=position_jitter(width=.0001,height=.0001)) + scale_x_continuous(breaks = c(0,1)) + stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE) + labs(x="Employment Status (0 or 1)", y="Likelihood of voting") + ggtitle("Effect of Unemployment on Voting Behaviour")
